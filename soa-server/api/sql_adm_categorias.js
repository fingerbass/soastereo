...
const TABLE_NAME = 'adm_categorias'

var getOne = function(empresa, idcategorias, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idcategorias = @idcategorias`;
    var request = new sql.Request(con);
    request.input('idcategorias', idcategorias);
    var queryTime = `sql getOne ${TABLE_NAME} ${idcategorias} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idcategorias == idcategorias) {
...
var deleteById = function(empresa, idcategorias, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idcategorias = @idcategorias`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idcategorias} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
