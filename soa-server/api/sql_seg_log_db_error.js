...
const TABLE_NAME = 'seg_log_db_error'

var getOne = function(empresa, iderror, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where iderror = @iderror`;
    var request = new sql.Request(con);
    request.input('iderror', iderror);
    var queryTime = `sql getOne ${TABLE_NAME} ${iderror} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].iderror == iderror) {
...
var deleteById = function(empresa, iderror, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where iderror = @iderror`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${iderror} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
