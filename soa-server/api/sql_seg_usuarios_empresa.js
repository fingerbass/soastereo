...
const TABLE_NAME = 'seg_usuarios_empresa'

var getOne = function(empresa, idusuarioempresa, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idusuarioempresa = @idusuarioempresa`;
    var request = new sql.Request(con);
    request.input('idusuarioempresa', idusuarioempresa);
    var queryTime = `sql getOne ${TABLE_NAME} ${idusuarioempresa} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idusuarioempresa == idusuarioempresa) {
...
var deleteById = function(empresa, idusuarioempresa, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idusuarioempresa = @idusuarioempresa`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idusuarioempresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
