...
const TABLE_NAME = 'adm_empresas'

var getOne = function(empresa, idempresa, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idempresa = @idempresa`;
    var request = new sql.Request(con);
    request.input('idempresa', idempresa);
    var queryTime = `sql getOne ${TABLE_NAME} ${idempresa} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idempresa == idempresa) {
...
var deleteById = function(empresa, idempresa, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idempresa = @idempresa`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idempresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
