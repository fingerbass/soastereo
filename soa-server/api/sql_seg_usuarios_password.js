...
const TABLE_NAME = 'seg_usuarios_password'

var getOne = function(empresa, idusuariopassword, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idusuariopassword = @idusuariopassword`;
    var request = new sql.Request(con);
    request.input('idusuariopassword', idusuariopassword);
    var queryTime = `sql getOne ${TABLE_NAME} ${idusuariopassword} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idusuariopassword == idusuariopassword) {
...
var deleteById = function(empresa, idusuariopassword, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idusuariopassword = @idusuariopassword`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idusuariopassword} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
