...
const TABLE_NAME = 'adm_proveedores'

var getOne = function(empresa, idproveedor, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idproveedor = @idproveedor`;
    var request = new sql.Request(con);
    request.input('idproveedor', idproveedor);
    var queryTime = `sql getOne ${TABLE_NAME} ${idproveedor} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idproveedor == idproveedor) {
...
var deleteById = function(empresa, idproveedor, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idproveedor = @idproveedor`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idproveedor} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
