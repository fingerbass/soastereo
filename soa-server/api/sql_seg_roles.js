...
const TABLE_NAME = 'seg_roles'

var getOne = function(empresa, idrol, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `select * from ${TABLE_NAME} where idrol = @idrol`;
    var request = new sql.Request(con);
    request.input('idrol', idrol);
    var queryTime = `sql getOne ${TABLE_NAME} ${idrol} ${empresa} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
    request.query(query, function(err, recordset){
      con.close();
      general.timeLog('end', queryTime)
      // console.log('recordset[0]', recordset)
      // if (recordset !== undefined && recordset[0].idrol == idrol) {
...
var getOneRol = function(empresa, usuario, callback){
...
var deleteById = function(empresa, idrol, callback){
  ...
  util.getConnection(empresa, function(err, con){
    ...
    var query = `delete from ${TABLE_NAME} where idrol = @idrol`;
    var request = new sql.Request(con);
    
    var queryTime = `sql deleteById ${TABLE_NAME} ${idrol} ` + Math.floor(Math.random() * 100000) + 1;
    general.timeLog('start', queryTime)
