...
.app-container
  .filter-container.text-align-right
    ...
    el-button.filter-item(v-waves='' :loading='cargandoInformacion' type='success' icon='el-icon-refresh' size='mini' @click='fetch()' :disabled="cargandoInformacion")
      | Actualizar
  el-table(ref="tblclientes" @selection-change="handleSelectionChange" :key='tableKey' v-loading='cargandoInformacion' :data='listaclientes'  :border='false' fit='' highlight-current-row='' height="calc(100vh - 220px)" max-height="calc(100vh - 220px)" style="width: 100%;" size='mini' @sortByName-change='sortChange' @current-change="handleCurrentChange" :row-class-name="tableRowClassName" @cell-dblclick= "dblhandlecurrentchange" @row-click="rowClicked" @row-dblclick="rowDblClicked" empty-text="No existen Registros")
...
import formclientes                               from '@/components/eventos'
