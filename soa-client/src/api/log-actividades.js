...
export function logactividadesLista(query) {
  ...
  return request({
    url   : process.env.VUE_APP_API_RUTA_LOG_ACTIVIDADES + '/lista',
    method: 'get',
    params: query
  }, function (err) {
    return err
...
export function logactividadesMantenimiento(data) {
  data.token    = getToken()
  ...
  return request({
    url   : process.env.VUE_APP_API_RUTA_LOG_ACTIVIDADES + '/mantenimiento',
    method: 'post',
    data
  }, function (err) {
    return err
